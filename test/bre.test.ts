...
describe("basic regular expressions", () => {
    test("matches BRE anchors, repetition, classes, and groups", () => {
        ...
        expect(find_bre(bytes_from_string("a\\c"), compile_bre("[\\]"))).toBeDefined();
        expect(find_bre(bytes_from_string("c\\d"), compile_bre("[\\^ab]"))).toBeDefined();
        expect(find_bre(bytes_from_string("ab0-]"), compile_bre("[][.-.]-0]"))).toBeDefined();
        expect(find_bre(bytes_from_string("Abc"), compile_bre("[[.a.]b]"))).toBeDefined();
        expect(find_bre(bytes_from_string("Abc"), compile_bre("[[=a=]b]"))).toBeDefined();
        expect(find_bre(bytes_from_string("ab0-]"), compile_bre("[A-[.].]c]"))).toBeDefined();
